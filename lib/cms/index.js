...
Cms.prototype.logon = function() {
  ...
  request.get(this.url('auth', 'logon'), this.params({ xmlData: xmlData }), function(error, response) {
    if (error) {
      d.reject(error);
      return;
    }
    ...
    var parser = new xml2js.Parser({ tagNameProcessors: [formatAuthTagName]});
...
Cms.prototype.logoff = function() {
  return getPromise(this.url('auth', 'logoff'), this.params());
};
...
Cms.prototype.getReportById = function(sourceId, qs) {
  return getPromise(this.url('reportData', 'report', sourceId), this.params(qs));
};
...
Cms.prototype.getPromptDescriptionById = function(sourceId, qs) {
  return getPromise(this.url('promptDescription', 'report', sourceId), this.params(qs));
};
